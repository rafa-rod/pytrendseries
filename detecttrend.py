...
def _calcula_MDD(precos):
    ...
    dd_maximo = 0
    maximo_cota = 0
    maxdd = pd.DataFrame(np.array([[np.nan, np.nan, np.nan, np.nan, 0]]), 
    ...
    for i in range(len(precos)):
        ...
        if dd_atual < dd_maximo:
            ...
            df = pd.DataFrame(np.array([[location_max, location_min, maximo_cota, cota, dd_maximo]]), 
                         columns=["Date Peak", "Date Valley", "Peak", "Valley", "MaxDD"])
...
def maxdrawdown(df_prices, window = None, **kwargs) -> pd.DataFrame:
    ...
    if window:
        for i in tqdm(range(0, df_prices.shape[0]+window, window)):
            try: dd = _calcula_MDD(df_prices[i:i+window])
            except: dd = _calcula_MDD(df_prices[i:])
...
def detecttrend(df_prices, trend: str = "downtrend", limit: int = 5, 
                        window: int = 21, **kwargs) -> pd.DataFrame:
    ...
    while True:
        price2 = prices[i]
        price1 = prices[i+1]
        if trend.lower()=="downtrend" and price1 < price2: go_trend=True
        elif trend.lower()=="uptrend" and price1 > price2: go_trend=True
        else: go_trend=False

        if go_trend:
            trend_df = np.empty([1, 6], dtype=object)
            try: found = df_array[i:(i+window)]
            except: found = df_array[i:] #if len(array)<window size
            ...
            if list(min_interval):
                ...
                if found2.size == 0: found2 = min_interval
                if trend.lower()=="downtrend":
                ...
                location_min = min_interval[np.where(min_interval==priceMin)[0],:][0][0]
                if trend.lower()=="downtrend":
    ...
    if trend == "downtrend":
        getTrend2["drawdown"] = [abs(getTrend2["price0"].iloc[x]-getTrend2["price1"].iloc[x])/max(getTrend2["price0"].iloc[x],getTrend2["price1"].iloc[x]) for x in range(getTrend2.shape[0])]
    elif trend == "uptrend":
        getTrend2["drawup"]  = [abs(getTrend2["price0"].iloc[x]-getTrend2["price1"].iloc[x])/min(getTrend2["price0"].iloc[x],getTrend2["price1"].iloc[x]) for x in range(getTrend2.shape[0])]  
