...
def random_integer_and_continuously_increasing_data(janela, trend, limit):
	...
	getTrend3, quantile = detectTrend(random_series, trend=trend, limit=limit,
	                                      window=janela)

	plot_trend(random_series, getTrend3, "random", trend)

	maxtrend = max_trend(random_series, "random", trend)
	plot_maxdrawdown(random_series, maxtrend, "random", trend, style="shadow")
...
def random_integer_and_continuously_decreasing_data(janela, trend, limit):
...
def sine_random_values_test(janela, trend, limit):
	x = np.linspace(1, 101)
	def f(x):
...
sine_random_values_test(janela, trend, limit)
